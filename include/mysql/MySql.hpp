...
public:
    ...
    template <typename T>
    void insert(T& items, const std::string& idname);

    template <typename T>
    void insert(const std::vector<T>& items);

    template <typename T>
    void insert(const std::vector<T>& items, const std::string& idname);
    ...
    template <typename T>
    T queryById(const std::string& idName, long long id);
...
template <typename T>
void MySql::insert(T& item, const std::string& idname) {
    std::stringstream ss;
    ...
    for(auto kv : name) {
        ...
        if(kv.first == idname) {
            vec.push_back("crushed-int-null");
        } else {
            vec.push_back(kv.second);
        }
    }
...
template <typename T>
T MySql::queryById(const std::string& idName, long long id) {
    std::stringstream ss;
    ss << "select * from " << table_ << " where " << idName << " = ? ";
    return query(ss.str()).bind({std::to_string(id)}).executeQueryOne<T>();
}
